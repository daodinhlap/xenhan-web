                    ...
                    exporters = {
                        xlsx: function (rDel, name) {
                            var rcMap = {},
                                dataURL = $rows.map(function (ir, val) {
                                    if (!!~ignoreRows.indexOf(ir) || $(val).is(ignoreCSS)) {
                                        return;
                                    }
                                    ...
                                    return [$cols.map(function (ic, val) {
                                        ...
                                        if (rcMap[ir]) {
                                            ...
                                            var value = Number($(val).text().replace(/\,/gi,''));
                                            return new Array(total).concat(isNaN(value)? $(val).text(): ($(val).text() == ''?'':value));
                                        }
                                        var value = Number($(val).text().replace(/\,/gi,''));
                                        return isNaN(value)? $(val).text(): ($(val).text() == ''?'':value);
                                    }).get()];
                                }).get(),
                        ...
                        csv: function (rdel, name) {
                            var colD = TableExport.prototype.csv.separator,
                                dataURL = $rows.map(function (i, val) {
                                    if (!!~ignoreRows.indexOf(i) || $(val).is(ignoreCSS)) {
                                        return;
                                    }
                                    var $cols = $(val).find('th, td');
                                    return $cols.map(function (i, val) {
