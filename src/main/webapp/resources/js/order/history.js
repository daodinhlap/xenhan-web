...
function Form() {
    ...
    this.different = function() {return $('#different').val()};
    this.setDifferent = function(value) {return $('#different').val(value)};
...
function cancelOrder(orderId) {
    ...
    noti.confirmWithBtn(
    		"<strong>Bạn muốn hủy đơn hàng?</strong><br>"+
    		"<select class='form-control' id='message-cancel'>" +
    		"<option>Chọn lý do hủy đơn hàng</option>"+
        	"<option>Tài xế đến lấy hàng trễ</option>" +
    		"<option>Khách đi công tác nhiều ngày</option>" +
    		"<option>Shop yêu cầu giao gấp</option>" +
    		"<option>Khách yêu cầu giao ngoài giờ làm việc</option>" +
    		"<option>Khách đổi hàng khác</option>" +
    		"<option>Shop chưa có hàng</option>" +
    		"<option id ='different'>Lý do khác </option>"+
    		"</select>"+
             "<textarea class='form-control' rows='2' id='message-cancel1' style='display: none' " +
                              "placeholder='Lý do hủy đơn?'></textarea>"+
                                "<p id='rest-char'></p>",
                        "Đồng ý", "Không đồng ý",
            $("#message-cancel").on("click",disableWhenDifferentNoChoice()),
...
function  disableWhenDifferentNoChoice() {
		 $('message-cancel1').removeAttr('style');  
}
